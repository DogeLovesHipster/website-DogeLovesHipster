    ...
    <footer className='sm:px-16 px-6 sm:py-16 py-10 max-x-7xl mx-auto relative z-0'>
      <div className="h-[80px] rounded-2xl bg-black-100  w-full flex flex-wrap items-center justify-center gap-5">
        ...
        <div className="flex justify-center items-center space-x-4 ">
          {contact.map((item) => (
            <a key={item.name} href={item.link} target="_blank" rel="noopener noreferrer" className="hover-target">
              <img src={item.icon} alt={item.name} className="h-8 w-8 hover-icon-enlarge" />
            </a>
          ))}
        </div>
